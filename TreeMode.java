...
public class TreeMode extends JPanel {
	...
	private static void createAndShowUI() {
		...
		TreeMode fd = new TreeMode(layers);
		//fd.practiceMode();
		//fd.teacherMode();
		fd.freedrawingMode();
		...
		frame.add(fd, BorderLayout.CENTER);
	...
	class TeacherPanel extends JPanel {
		...
		public void checkanswer(){
			...
			if(x.equals(y)){
				...
				for(NodeLabel xf:newtree.list){
					if(!xf.gerror.equals("")){
						System.out.println("gerror is"+xf.gerror);
						ee=1;
				break;
					}
				}
				System.out.println("one wrong added");
				nownode.correct=-1;
				if(ee==1){
		...
		public TeacherPanel() {
			...
			sn.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					System.out.println("save and new");
					saveTree();
					tree = new Tree();
...
//				sentenceTree.sentence.setSelectionPath(new TreePath(now));
	...
	class Tree {
		...
		public void setLocation2(Rectangle bounds, Plain drawingPanel) {
			...
			for (NodeLabel cc : basement) {
				...
				drawingPanel.drawroom.back.add(bb);
	...
	public void forInstructionEdit() {
		System.out.println("TeacherPanel instructionEdit");
	...
	public void forSentenceEdit() {
		System.out.println("TeacherPanel SentenceEdit");
