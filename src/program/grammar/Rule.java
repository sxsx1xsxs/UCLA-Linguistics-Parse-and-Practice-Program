...
public class Rule {
	 ...
	 */
	public boolean obeyRule(String mother, Vector<String> children){
		if(!head.node.equals(mother)){
			return false;
		}
		else
		{
	...
	//used when constructing a rule containing only head in Grammar
				...
				if(now instanceof Unit){
					...
					if(nowUnit.position=="right"){
						...
						//the situations that have left counterparts
							...
							if(!left.property.equals(nowUnit.property)||!left.position.equals("left")){
								error="There s a "+nowUnit.node+" that cannot be applied";
								return;
							}else{
								process.add(complex);
