...
public class SentenceTree extends JPanel {
	...
	DefaultMutableTreeNode presentNode;
	...
	// the following three hold components in TreeMode in order to connect
	// actions
	TreeMode.TeacherPanel teacherPanel;
	Plain treeModePlain;

	// the same status as in TreeMode, either "Add" or "Review"
	// will be changed in TreeMode "review" and "add" label (as button)
	String status = "Add";
	...
	public void selectNode() {
		...
		if (presentNode != sentence.root) {
			...
			System.out.println("SelectNonRoot");
			...
			// Practice Mode
				...
				System.out.println("here is practice mode");
				if (present.type.equals("sentence")) {
					System.out.println(present.correct+"correctness");
	...
	public SentenceTree(TreeMode treeModeInput) {
...
//				System.out.println("row" + row + "selected");
	...
	public void addCategory() {
		...
		if (info != null) {
			...
			if (presentNode.equals(sentence.root)) {
				...
				((DefaultTreeModel) sentence.getModel()).nodeStructureChanged((TreeNode) presentNode);
				...
				((DefaultTreeModel) sentence.getModel()).nodeStructureChanged((TreeNode) presentNode.getParent());
			}
			sentence.resetIndex();
	...
	public void addSentence(TreeMode.Tree tree, DefaultMutableTreeNode node) {
		System.out.println("st" + 14);
		IndexNode addedSentence = new IndexNode(tree);
		if (node == sentence.root) {
			node.add(addedSentence);
			((DefaultTreeModel) sentence.getModel()).nodeStructureChanged((TreeNode) node);
			sentence.resetIndex();
			sentence.expandPath(new TreePath(node.getPath()));
		} else {
			...
			((DefaultTreeModel) sentence.getModel()).nodeStructureChanged((TreeNode) parent);
			sentence.resetIndex();
	...
	public void changeSentence(TreeMode.Tree tree, DefaultMutableTreeNode node) {
	...
	class STree extends JTree {
		DefaultMutableTreeNode root;
		IndexNode instructionNode;
...
//		        String node = (String) ((DefaultMutableTreeNode) value).getUserObject();
		        ...
		        if (leaf) {
		            ...
		            setBackground(Color.decode("0x99FF99"));}
		        		...
		        		System.out.println("indexnode cast exception");
		        	}
		        }
		...
		public void resetIndex(IndexNode node) {
			System.out.println("st" + 18);
			for (int i = 0; i < node.getChildCount(); i++) {
				IndexNode child = (IndexNode) node.getChildAt(i);
				child.changeIndex(node.index + i + ".");
				((DefaultTreeModel) getModel()).nodeStructureChanged((TreeNode) child);
			}
		}
		...
		public void updateATreenNode(IndexNode now, TreeMode.Tree tree) {
			// TODO Auto-generated method stub
			now.updateTree(tree);
		...
		public PopUp(String type) {
			...
			delete.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					DefaultMutableTreeNode next = (DefaultMutableTreeNode) presentNode.getNextSibling();
					if (next == null) {
						next = (DefaultMutableTreeNode) presentNode.getPreviousSibling();
					}
					if (next == null) {
						next = parent;
					}
	...
	public void updateATreeNode(IndexNode now, TreeMode.Tree tree) {
